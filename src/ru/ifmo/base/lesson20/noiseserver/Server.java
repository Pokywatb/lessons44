...
// сселекторы могут работать с несколькими каналами, прри этом канал регистрируется в селекторе и селектор начинает следить за тем, что происходит в канале
// события в канале:
//- connect - клиент подключился к серверу
//- read - нужно читать данные
//- write - нужно записать данные
//- accept - ктото подключился к серверу
public class Server {
...
// ServerSocketChannel на указанном порту жждет подключений
            // открываем канал сервера
            serverChannel = ServerSocketChannel.open();
            // слушаем определенный порт (8090)
            serverChannel.bind(new InetSocketAddress(8090));// указали порт
            // перевод в неблокирующий режим
            serverChannel.configureBlocking(false); // перевели канал в не блокирующий режим, теперь к серверу могут подключиться неограниченное кол-во клиентов.
            // регистрируем канал в селекторе на все допустимые события (для serverSocket - OP_ACCEPT)
            serverChannel.register(selector, serverChannel.validOps());// регистрируем канал в селектора на любые события validOps()
        }

        @Override
        protected void loop() throws IOException {
            // ожидание новых событий
            selector.select();// селектор ожидает новых событий
            ...
            while (iterator.hasNext()){
                ...
                // флаг означает, что пришли данные для чтения
                    ...
                    readData((SocketChannel)key.channel());//метод channel вернет канал в котором произошло событие
                }
        ...
        private void readData(SocketChannel channel){
            ...
            try {
                ...
                // сервер говорит, что каналу доступна операция записи.
